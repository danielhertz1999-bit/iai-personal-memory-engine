...
Contracts covered:
  ...
  W1 — single enqueue + await resolves within the coalesce window and
       produces exactly one tbl.add() call containing one record.
  W2 — two enqueues within <coalesce_ms> land in ONE tbl.add call (batch
       size 2). This is the core "coalesce" invariant.
  W3 — max_batch+1 enqueues produce TWO tbl.add calls (max_batch split).
  W4 — stop() drains an in-flight batch: pending enqueues complete before
       stop() returns; further enqueue attempts raise RuntimeError.
  W5 — back-pressure: with max_queue_size=N, the (N+1)th enqueue awaits
       until a flush drains the queue (never unbounded growth).
  W6 — on_flushed callback fires once per record, in batch order.
  W7 — tbl.add() raises -> every pending future in that batch resolves
       with the same exception; queue stays running so subsequent
       enqueues still work.
"""
...
class MockAsyncTable:
    ...
    exception via ``raise_on_add`` to test W7.
    """
    ...
    async def add(self, batch) -> None:
        # Copy so later mutations by the queue don't change what we recorded.
...
def test_coalesce_window_batches_concurrent_enqueues():
    """Two enqueues inside the same coalesce window -> one tbl.add(size=2)."""
    table = MockAsyncTable()
    ...
    asyncio.run(run())
    # Exactly ONE add() call carrying both records, in enqueue order.
...
def test_max_batch_splits_into_two_flushes():
    """max_batch+1 enqueues -> two add() calls (one full, one size=1)."""
    table = MockAsyncTable()
    ...
    # Either [4,1] (strict split) or [5] would violate max_batch. We assert
    # the total is 5 and at least one batch is <= max_batch=4.
...
def test_stop_drains_pending_records():
    """stop() awaits the in-flight batch so enqueued records are durable."""
    table = MockAsyncTable()
    ...
    async def run() -> None:
        ...
        await q.start()
        fut = await q.enqueue({"id": "r1"})
        # Don't await fut here -- let stop() drain it.
        await q.stop()
        assert fut.done(), "stop() must drain pending futures"
        # Enqueuing after stop should fail.
...
def test_backpressure_awaits_when_queue_full():
    """max_queue_size=2 -> third enqueue awaits until a flush frees a slot."""
    table = MockAsyncTable()
    # Slow down the flush so back-pressure is observable.
    table._delay_s = 0.05
    ...
    async def run() -> int:
        ...
        await q.start()
        try:
            # First two are accepted immediately (fit inside the buffer).
            f1 = await q.enqueue({"id": "r1"})
            f2 = await q.enqueue({"id": "r2"})
            # Third one MUST await at least one flush before accepting.
...
def test_flush_exception_propagates_to_all_futures_in_batch():
    ...
    async def run() -> tuple[list, MockAsyncTable]:
        ...
        try:
            ...
            # Queue must stay running; clear the error and try again.
