"""AUTIST-13 — 14th autistic-kernel profile knob FLIP verification.

The 14th autistic-kernel knob `camouflaging_relaxation` is FLIPPED from phase=3
(deferred) to phase=1 (live). (MCP-12) subsequently appends the 15th
sealed operator-facing knob `wake_depth`, so PHASE_1_LIVE=15 post-.
...
def test_phase_1_live_is_14():
    """10 autistic-kernel + wake_depth (MCP-12) = 11 live.

    Test name kept for git stability. Verifies camouflaging_relaxation
    (AUTIST-13) remained live after flipped it.
    """
