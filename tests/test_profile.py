...
Covers:
- Registry shape: 15 total, 15 live, 0 Phase-2 deferred, 0 Phase-3 deferred.
    ...
    for name, spec in PROFILE_KNOBS.items():
        if name == "wake_depth":
            assert spec.requirement_id == "MCP-12", (
                f"wake_depth must carry requirement_id, got {spec.requirement_id}"
            )
            continue
...
def test_live_knob_defaults_match_d11():
    """D-11 specifies autistic-kernel defaults on the 4 live knobs."""
    state = default_state()
