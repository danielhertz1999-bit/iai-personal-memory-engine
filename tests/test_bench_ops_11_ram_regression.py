...
CI-runnable guard for bench/memory_footprint.py. The large-N target
(RSS <= 300 MB at N=10k warm on a 16+ GB machine) runs ad-hoc; this test
exercises the small-N path (N=100-500 with a 64d embedding) so CI catches
harness drift without spinning up a 10k-record store.

See bench/memory_footprint.py — the harness under guard.
"""
...
def test_memory_footprint_small_n_under_threshold(tmp_path: Path):
    ...
    The 1600 MB N-aware target is the current threshold (revised from 300 MB
    when LanceDB was replaced with HippoDB + hnswlib). This guard only asserts
    that the harness returns a plausible positive reading and respects the
    JSON schema the bench report consumes.
    """
